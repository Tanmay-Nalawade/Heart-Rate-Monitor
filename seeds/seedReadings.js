...
function generateSpO2(baseSpo2 = 97, variation = 2) {
  return Math.round(
    Math.min(
      100,
      Math.max(90, baseSpo2 + (Math.random() - 0.5) * variation * 2)
    )
  );
}
...
const seedReadings = async () => {
  try {
    ...
    if (!device) {
      ...
      if (!device) {
        console.log(
          "No devices found in database. Please create a device first."
        );
        return;
      }
      ...
      console.log(
        `Linking device ${device.hardwareId} to patient ${patient.email}`
      );
    ...
    console.log(
      `Seeding readings for patient: ${patient.email}, device: ${device.hardwareId}`
    );
    ...
    for (let d = days - 1; d >= 0; d--) {
      ...
      for (let i = 0; i < perDay; i++) {
        const readingTime = new Date(
          dayStart.getTime() + i * (24 / perDay) * 60 * 60 * 1000
        );
    ...
    console.log(
      `  Time range: ${readings[0].readingTime.toLocaleDateString()} to ${readings[
        readings.length - 1
      ].readingTime.toLocaleDateString()}`
    );
    console.log(
      `  Heart rate range: ${Math.min(
        ...readings.map((r) => r.heartRate)
      )}-${Math.max(...readings.map((r) => r.heartRate))} bpm`
    );
    console.log(
      `  SpO2 range: ${Math.min(...readings.map((r) => r.spo2))}-${Math.max(
        ...readings.map((r) => r.spo2)
      )}%`
    );
  } catch (err) {
